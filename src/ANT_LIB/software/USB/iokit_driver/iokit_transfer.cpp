...
IOKitTransfer* IOKitTransfer::MakeControlTransfer(const IOKitDeviceHandle& dev_handle_, struct libusb_control_setup& setup_,
                                              unsigned char data_buffer_[], unsigned int timeout_)
{
   return new IOKitTransfer(dev_handle_, setup_, data_buffer_, TransferType::CONTROL, timeout_);
}

IOKitTransfer* IOKitTransfer::MakeInterruptTransfer(const IOKitDeviceHandle& dev_handle_, unsigned char endpoint_,
                             unsigned char data_buffer_[], unsigned long data_length_, unsigned int timeout_)
{
   return new IOKitTransfer(dev_handle_, endpoint_, data_buffer_, data_length_, TransferType::INTERRUPT, timeout_);
}
...
IOKitTransfer* IOKitTransfer::MakeBulkTransfer(const IOKitDeviceHandle& dev_handle_, unsigned char endpoint_,
                             unsigned char data_buffer_[], unsigned long data_length_, unsigned int timeout_)
{
   return new IOKitTransfer(dev_handle_, endpoint_, data_buffer_, data_length_, TransferType::BULK, timeout_);
}
   ...
   dev_handle(dev_handle_)
{
   ...
   DSIThread_CondInit(&stConditionCallbackComplete);
   return;
}
...
IOKitTransfer::~IOKitTransfer()
{
   ...
   DSIThread_CondDestroy(&stConditionCallbackComplete);
}
...
IOKitError::Enum IOKitTransfer::Submit()
{
   ...
   if(transfer_complete)   //Check if we were cancelled
   {
...
{
   TransferStatus::Enum ret = TransferStatus::ERROR;
   //Wait for response
   DSIThread_MutexLock(&stMutexCriticalSection);

   if(!transfer_submitted) //If calling receive before submit, kill everything and return error
   {
      transfer_complete = TRUE;
      ret = TransferStatus::ERROR;
   }
   else
   {
      if(!transfer_complete)
      {
         if(!DSIThread_CondTimedWait(&stConditionCallbackComplete, &stMutexCriticalSection, DSI_THREAD_INFINITE))
            ret = TransferStatus::INTERRUPTED;
      }

      if(transfer_complete)
      {
         ret = HandleCallback(result_result, result_io_size);
      }
   }
...
{
   IOKitError::Enum ret = IOKitError::SUCCESS;

   DSIThread_MutexLock(&stMutexCriticalSection);

   if(!transfer_submitted) //If the transfer is not submitted, just kill it now
   {
      transfer_complete = TRUE;
      ret = IOKitError::TIMEOUT;
   }
   ...
   {
      //Tell the transfer it needs to die
      if(!transfer_complete)
      {
         switch(type)
         {
            case TransferType::CONTROL:
               ret = CancelControlTransfer();
               break;
            case TransferType::BULK:
            case TransferType::INTERRUPT:
         ...
         if(ret == IOKitError::SUCCESS)
         {
            //Now wait until we see it cancelled
            if(!DSIThread_CondTimedWait(&stConditionCallbackComplete, &stMutexCriticalSection, DSI_THREAD_INFINITE))
               ret = IOKitError::INTERRUPTED;
         }
      }
   }
...
void IOKitTransfer::Callback(void* refcon, IOReturn result, void* arg0)
{
   ...
   This->result_result = result;
   This->result_io_size = (UInt32)(size_t)arg0;


   This->transfer_complete = TRUE;
   DSIThread_CondBroadcast(&(This->stConditionCallbackComplete));
