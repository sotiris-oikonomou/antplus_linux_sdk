...
#ifndef USB_DEVICE_LIBUSB_MAC_HPP
#define USB_DEVICE_LIBUSB_MAC_HPP

#include "types.h"

#include "dsi_libusb_library_mac.hpp"
...
{
  public:
   USBDeviceLibusb(struct libusb_device_node& stDevice_);
   USBDeviceLibusb(const USBDeviceLibusb& clDevice_);

   USBDeviceLibusb& operator=(const USBDeviceLibusb& clDevice_);

   struct libusb_device& GetRawDevice() const { return *pstDevice; }
  ...
  private:
   ...
   struct libusb_device* pstDevice; //should we copy instead?
   USHORT usVid;
   USHORT usPid;
...
#endif // !defined(USB_DEVICE_LIBUSB_MAC_HPP)
